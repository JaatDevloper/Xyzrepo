...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8490889034:AAEVf9rC5DedCwxbjHBNvr0qncxvUM2Knc4")
...
async def fetch_pwwp_data(url: str, headers: Dict = None, params: Dict = None, data: Dict = None, method: str = 'GET') -> Any:
    max_retries = 3
...
async def process_pwwp_chapter_content(session: aiohttp.ClientSession, chapter_id, selected_batch_id, subject_id, schedule_id, content_type, headers: Dict):
    ...
    data = await fetch_pwwp_data(url, headers=headers)
    content = []
...
async def get_pwwp_all_todays_schedule_content(session: aiohttp.ClientSession, selected_batch_id: str, headers: Dict) -> List[str]:
    ...
    todays_schedule_details = await fetch_pwwp_data(url, headers)
    all_content = []
...
async def process_pwwp(bot: Client, m: Message, user_id: int):
    ...
    async with aiohttp.ClientSession(connector=CONNECTOR, loop=loop) as session:
        try:
            ...
            courses = await fetch_pwwp_data(url, headers)
